...
/**
 * copy from com.github.zagum:Android-ExpandIcon:1.3.0
 * @modify by Z-P-J
 */
public class ExpandIcon extends View {
    ...
    public ExpandIcon(@NonNull Context context) {
        this(context, null);
    }

    public ExpandIcon(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ExpandIcon(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        TypedArray array = getContext().getTheme().obtainStyledAttributes(attrs,
                R.styleable.ExpandIcon,
                0, 0);

        final boolean roundedCorners;
        final long animationDuration;
        try {
            roundedCorners = array.getBoolean(R.styleable.ExpandIcon_eiv_roundedCorners, false);
            switchColor = array.getBoolean(R.styleable.ExpandIcon_eiv_switchColor, false);
            color = array.getColor(R.styleable.ExpandIcon_eiv_color, Color.BLACK);
            colorMore = array.getColor(R.styleable.ExpandIcon_eiv_colorMore, Color.BLACK);
            colorLess = array.getColor(R.styleable.ExpandIcon_eiv_colorLess, Color.BLACK);
            colorIntermediate = array.getColor(R.styleable.ExpandIcon_eiv_colorIntermediate, -1);
            animationDuration = array.getInteger(R.styleable.ExpandIcon_eiv_animationDuration, (int) DEFAULT_ANIMATION_DURATION);
            padding = array.getDimensionPixelSize(R.styleable.ExpandIcon_eiv_padding, -1);
     ...
     */
    public void setState(@ExpandIcon.State int state, boolean animate) {
        this.state = state;
        if (state == MORE) {
            fraction = 0f;
        } else if (state == LESS) {
